...
pub fn kat_export(declarations: &Vec<Declaration>) -> String {
    ...
    for q in declarations {
        ...
        if !hs.contains_key(&startend) {
                ...
                s.push_str(&format!("<rdf:Description rdf:nodeID=\"{}\"><kat:run rdf:nodeID=\"kat_run\"/><kat:kannspec rdf:nodeID=\"KAT_Declarations_KAnnSpec\"/><kat:concept>Restriction</kat:concept><kat:type rdf:resource=\"http://jfschaefer.de/declarations/KAnnSpec#Restriction\"/><kat:annotates rdf:resource=\"http://localhost:3000/content/test.html#cse(%2F%2F*%5B%40id%3D'{}'%5D%2C%2F%2F*%5B%40id%3D'{}'%5D%2C%2F%2F*%5B%40id%3D'{}'%5D)\"/><d:restrictiontype rdf:resource=\"http://jfschaefer.de/declarations/KAnnSpec#definingrestriction\"/></rdf:Description>", &restrictionkatid, q.sentence.get_property("id").unwrap(), q.restriction_start.get_property("id").unwrap(), q.restriction_end.get_property("id").unwrap()));
        ...
        s.push_str(&format!("<rdf:Description rdf:nodeID=\"{}\"><kat:run rdf:nodeID=\"kat_run\"/><kat:kannspec rdf:nodeID=\"KAT_Declarations_KAnnSpec\"/><kat:concept>Identifier</kat:concept><kat:type rdf:resource=\"http://jfschaefer.de/declarations/KAnnSpec#Identifier\"/><kat:annotates rdf:resource=\"http://localhost:3000/content/test.html#cse(%2F%2F*%5B%40id%3D'{}'%5D%2C%2F%2F*%5B%40id%3D'{}'%5D%2C%2F%2F*%5B%40id%3D'{}'%5D)\"/><d:restrictedby rdf:nodeID=\"{}\"/><d:identifierisseqtype rdf:resource=\"http://jfschaefer.de/declarations/KAnnSpec#{}\"/></rdf:Description>", declid, mathnode, q.var_start.get_property("id").unwrap(), q.var_end.get_property("id").unwrap(), restrictionkatid,
        match q.idtype {
