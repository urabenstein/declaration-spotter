...
pub fn kat_export(declarations: &Vec<Declaration>) -> String {
    ...
    for q in declarations {
        ...
        s.push_str(&format!("<rdf:Description rdf:nodeID=\"{}\"><kat:run rdf:nodeID=\"kat_run\"></kat:run><kat:kannspec rdf:nodeID=\"KAT_Declarations_KAnnSpec\"></kat:kannspec><kat:concept>Identifier</kat:concept><kat:type rdf:resource=\"http://jfschaefer.de/declarations/KAnnSpec#Identifier\"></kat:type><kat:annotates rdf:resource=\"http://localhost:3000/content/test.html#cse(%2F%2F*%5B%40id%3D'{}'%5D%2C%2F%2F*%5B%40id%3D'{}'%5D%2C%2F%2F*%5B%40id%3D'{}'%5D)\"></kat:annotates><d:restrictedby rdf:nodeID=\"{}\"/><d:identifierisseqtype rdf:resource=\"http://jfschaefer.de/declarations/KAnnSpec#{}\"/></rdf:Description>", declid, mathnode, q.var_start.get_property("id").unwrap(), q.var_end.get_property("id").unwrap(), restrictionkatid,
        match q.idtype {
